| № | Branch name | Russian name | Generic command |
|---|---|---|---|
| 1 | `{agent}/project-structure` | Структура проекта | `Project Structure Overview` |
| 2 | `{agent}/build` | Сборка проекта | `Run the build and report the result` |
| 3 | `{agent}/rename-method` | Переименование метода | `Rename FindBooksByNameAsync to FindBooksAsync` |
| 4 | `{agent}/implement-aaa-in-tests` | Реализация AAA в тестах | `Implement Arrange Act Assert pattern in tests` |
| 5 | `{agent}/add-status-endpoint` | Добление эндпоинта Status | `add status endpoint that returns GetStatusResponseDto object with fields { IsActtive }` |
