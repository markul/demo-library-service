...
public class LibraryApiFactory : WebApplicationFactory<Program>
{
    ...
    private sealed class FakeEbookCatalogService : IEbookCatalogService
    {
        ...
        public Task<IReadOnlyCollection<EbookSearchResultDto>> FindBooksByNameAsync(
            string name,
            CancellationToken cancellationToken = default)
        {
            var books = Books
                .Where(x => x.Title.Contains(name, StringComparison.OrdinalIgnoreCase))
                .Select(x => new EbookSearchResultDto(x.Id, x.Title))
                .ToArray();

            return Task.FromResult<IReadOnlyCollection<EbookSearchResultDto>>(books);
        }
    }
}
