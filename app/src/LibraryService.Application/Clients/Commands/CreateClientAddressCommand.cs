...
public record CreateClientAddressCommand(Guid ClientId, string City, string Country, string Address, string PostalCode) : IRequest<ClientAddressDto>;
...
public class CreateClientAddressCommandHandler : IRequestHandler<CreateClientAddressCommand, ClientAddressDto>
{
    ...
    public async Task<ClientAddressDto> Handle(CreateClientAddressCommand request, CancellationToken cancellationToken)
    {
        ...
        return new ClientAddressDto(created.Id, created.ClientId, created.City, created.Country, created.Address, created.PostalCode);
    }
}
